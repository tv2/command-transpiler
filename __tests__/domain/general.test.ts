...
test('greater (number)', () => {
  const condition = new Condition('text | greater 5')
  expect(condition.check({ text: '' })).toBeFalsy()
  expect(condition.check({ text: 0 })).toBeFalsy()
  expect(condition.check({ text: 5 })).toBeFalsy()
  expect(condition.check({ text: -5 })).toBeFalsy()
  expect(condition.check({ text: -6 })).toBeFalsy()
  expect(condition.check({ text: -900 })).toBeFalsy()
  expect(condition.check({ text: 6 })).toBeTruthy()
  expect(condition.check({ text: 900 })).toBeTruthy()
  expect(condition.check({ text: "6" })).toBeTruthy()
  expect(condition.check({ text: null })).toBeFalsy()
...
test('greater (variable)', () => {
  const condition = new Condition('text | greater limit')
  expect(condition.check({ limit: 5, text: '' })).toBeFalsy()
  expect(condition.check({ limit: 5, text: 0 })).toBeFalsy()
  expect(condition.check({ limit: 5, text: 5 })).toBeFalsy()
  expect(condition.check({ limit: 5, text: -5 })).toBeFalsy()
  expect(condition.check({ limit: 5, text: -6 })).toBeFalsy()
  expect(condition.check({ limit: 5, text: -900 })).toBeFalsy()
  expect(condition.check({ limit: 5, text: 6 })).toBeTruthy()
  expect(condition.check({ limit: 5, text: 900 })).toBeTruthy()
  expect(condition.check({ limit: 5, text: "6" })).toBeTruthy()
  expect(condition.check({ limit: 5, text: null })).toBeFalsy()
  expect(condition.check({ limit: 5, text: 'hello' })).toBeFalsy()
  expect(condition.check({ limit: 5, text: 'shello' })).toBeFalsy()
  expect(condition.check({ limit: 5, text: 'hellos' })).toBeFalsy()
  expect(condition.check({ limit: 5, text: 'shellos' })).toBeFalsy()
})

test('less (number)', () => {
  const condition = new Condition('text | less 5')
...
test('less (variable)', () => {
  const condition = new Condition('text | less limit')
  expect(condition.check({ limit: 5, text: '' })).toBeTruthy()
  expect(condition.check({ limit: 5, text: 0 })).toBeTruthy()
  expect(condition.check({ limit: 5, text: 5 })).toBeFalsy()
  expect(condition.check({ limit: 5, text: -5 })).toBeTruthy()
  expect(condition.check({ limit: 5, text: -6 })).toBeTruthy()
  expect(condition.check({ limit: 5, text: -900 })).toBeTruthy()
  expect(condition.check({ limit: 5, text: 6 })).toBeFalsy()
  expect(condition.check({ limit: 5, text: 900 })).toBeFalsy()
  expect(condition.check({ limit: 5, text: "6" })).toBeFalsy()
  expect(condition.check({ limit: 5, text: null })).toBeTruthy()
  expect(condition.check({ limit: 5, text: 'hello' })).toBeFalsy()
  expect(condition.check({ limit: 5, text: 'shello' })).toBeFalsy()
  expect(condition.check({ limit: 5, text: 'hellos' })).toBeFalsy()
  expect(condition.check({ limit: 5, text: 'shellos' })).toBeFalsy()
